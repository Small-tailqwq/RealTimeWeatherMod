...
```ini
[Automation]
## Enable automatic seasonal scenery easter eggs
EnableSeasonalEasterEggs = true
## Enable automatic ambient sound management
...
[TimeSync]
## Enable time sync (Day/Sunset/Night auto control)
EnableTimeSync = true
...
- Fixed seasonal automation mistakenly turning off manually enabled scenery such as Sakura.
- Split seasonal scenery automation from ambient sound automation with `Automation.EnableAmbientSounds = false`.
- Narrowed automatic Sakura season to March-April, so May no longer auto-enables Sakura.

### v5.2.1 - Weather/Time Decouple + Cutscene Guard

- Decoupled weather sync from time sync and added `TimeSync.EnableTimeSync` (default `true`).
- Introduced centralized policy snapshot flow for environment control decisions.
- Fixed sync interference during cutscenes (both environment control and scenery automation are guarded).
- Improved weather UI behavior:
  - Turning weather UI on now refreshes immediately.
  - Valid cache is reused first; if no valid cache exists, fetch happens immediately.
  - Less need to press `F7` after rapid UI toggle changes.
- Added pure-logic tests for policy combinations and weather UI text state transitions.
